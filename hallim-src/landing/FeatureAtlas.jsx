...
export function HallimFeatureSystem({authHref}){
 ...
 return <div className={s.wrap} id="features">
  <section className={s.atlas} aria-labelledby="atlas-heading">
   <div className={s.atlasIntro}><div><p className={s.eyebrow}>Five inputs. One place to grow.</p><h2 id="atlas-heading">Not five disconnected tools.<br/><em>One learning journey.</em></h2></div><p>Lessons, practice, language tools, study checks and review all contribute to a clearer next step. Select an input to see how the parts connect.</p></div>
   <div className={s.graph} aria-label="Five parts of Hallim connect to Hallim Intelligence">
    ...
    <div className={s.hub}><div className={s.hubTop}><span className={s.hubMark}>ㅎ</span><span><small>ONE CONNECTED DESTINATION</small><strong>Hallim Intelligence</strong></span><span className={s.hubSpark}>✳</span></div><div className={s.hubFocus} key={chosen.key}><span className={s.hubLabel}>Connecting {chosen.number} · {chosen.title}</span><h3>{chosen.description}</h3><p>{chosen.detail}</p><a href={chosen.href}>Explore this feature <Arrow/></a></div><div className={s.hubOutcome}><span>YOUR NEXT MOVE, INFORMED BY YOUR WORK</span><div><i>1</i> Continue learning</div><div><i>2</i> Review what needs work</div><div><i>3</i> Recheck your understanding</div></div><small className={s.illustration}>Illustrative flow. AI suggestions depend on a signed-in learner’s available evidence.</small></div>
   </div>
