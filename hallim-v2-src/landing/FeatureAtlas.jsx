...
const intelligence=[
 ...
 ["✓","Fresh AI checkpoint","Generate a new mastery check based on studied material."],
 ["◈","Level promotion audit","Review the available evidence for moving beyond the selected level."]
];
const bands=[
...
export function HallimFeatureSystem({authHref}){
 ...
 return <div className={s.wrap} id="features">
  <section className={s.atlas} aria-labelledby="atlas-heading">
   <div className={s.atlasIntro}><div><p className={s.eyebrow}>Everything works together</p><h2 id="atlas-heading">Five ways to learn.<br/><em>One clearer next step.</em></h2></div><p>Every lesson, study check and review tells part of the story. Click any input to see how Hallim brings your learning evidence together.</p></div>
   <div className={s.graphFrame}>
    <div className={s.graphTop}><span><i aria-hidden="true"/> THE LEARNING CIRCUIT</span><span>Choose an input to see where it goes ↓</span></div>
    <div className={s.graph} aria-label="Five learning inputs connect to one Hallim Intelligence learning route">
     <div className={s.signalList} aria-label="Choose a learning signal">
      {signals.map((item,i)=><button key={item.key} onClick={()=>setFocus(i)} type="button" className={focus===i?s.signalActive:s.signal} aria-pressed={focus===i}><span className={s.signalNumber}>{item.number}</span><span className={s.signalIcon} aria-hidden="true">{item.icon}</span><span className={s.signalText}><strong>{item.title}</strong><small>{item.subtitle}</small></span><span className={s.signalArrow} aria-hidden="true">↗</span></button>)}
     </div>
     <svg className={s.wires} viewBox="0 0 170 490" preserveAspectRatio="none" aria-hidden="true">{[49,147,245,343,441].map((y,i)=><g key={i}><path className={focus===i?s.wireActive:s.wire} d={`M 0 ${y} C 88 ${y}, 68 245, 170 245`}/><circle cx="1" cy={y} r="4" className={focus===i?s.dotActive:s.dot}/></g>)}<circle cx="167" cy="245" r="7" fill="#3158d8" stroke="#f3f7ff" strokeWidth="3"/></svg>
     <div className={s.hub}>
      <div className={s.hubTop}><span className={s.hubMark}>ㅎ</span><span><small>ONE PLACE TO MAKE SENSE OF IT</small><strong>Hallim Intelligence</strong></span><span className={s.hubSpark} aria-hidden="true">✳</span></div>
      <div className={s.hubFocus} key={chosen.key}><span className={s.hubLabel}>Selected input <b>{chosen.number}</b> / {chosen.title}</span><h3>{chosen.description}</h3><div className={s.evidenceBox}><span>THIS INPUT BRINGS</span><p>{chosen.detail}</p></div><a href={chosen.href}>Explore the feature <Arrow/></a></div>
      <div className={s.hubOutcome}><span>TO MAKE THE NEXT STEP CLEARER</span><div className={s.routePills}><span><i>1</i> Learn</span><b aria-hidden="true">→</b><span><i>2</i> Review</span><b aria-hidden="true">→</b><span><i>3</i> Recheck</span></div></div>
      <small className={s.illustration}>Illustration only. Suggestions depend on the evidence a signed-in learner actually has.</small>
     </div>
    </div>
   </div>
   <div className={s.atlasFooter}><span><b>5</b> connected learning inputs</span><span><b>1</b> personal route</span><span><b>6</b> AI study tools</span><a href={entry}>Step into Hallim <Arrow/></a></div>
  </section>

  <section className={s.curriculum} id="curriculum">
   <div className={s.deepCopy}><p className={s.eyebrow}>The learning roadmap</p><h2>From <Word>안녕하세요</Word> to thoughts that sound like you.</h2><p>You shouldn’t have to guess which lesson belongs next. Hallim asks where you are and where you want to go, then opens a route through its published curriculum.</p><div className={s.bigNumbers}><span><b>15</b><small>published units</small></span><span><b>76</b><small>lessons & checkpoints</small></span><span><b>5</b><small>learning bands</small></span></div><a href={entry} className={s.featureLink}>Find your starting point <Arrow/></a></div>
