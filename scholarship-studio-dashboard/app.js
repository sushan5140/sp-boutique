...
const STORAGE_KEY = 'scholarship-studio-dashboard-v1';
const UNIVERSITY = {
...
function readState(){try{const old=JSON.parse(localStorage.getItem(STORAGE_KEY));if(old&&typeof old==='object')return {profile:Object.assign({},DEFAULT_PROFILE,old.profile||{}),apps:Array.isArray(old.apps)?old.apps.filter(a=>byId(a.id)):[],compare:Array.isArray(old.compare)?old.compare.filter(id=>byId(id)).slice(0,3):[],demo:old.demo===true};}catch(e){}return {profile:{...DEFAULT_PROFILE},apps:[],compare:[],demo:false};}
let state=readState();
let view=['dashboard','discover','eligibility','applications','compare'].includes(location.hash.slice(1))?location.hash.slice(1):'dashboard';
...
function syncShell(){
 ...
 $('#crumb-current').textContent=({dashboard:'OVERVIEW',discover:'DISCOVER',eligibility:'MY ELIGIBILITY',applications:'APPLICATIONS',compare:'COMPARE'})[view];
...
function navigate(next){if(!['dashboard','discover','eligibility','applications','compare'].includes(next))return;view=next;location.hash=next;render();window.scrollTo({top:0,behavior:'instant'});}
...
/* Post-login-style overview; all displayed personal counts come from local preview
   state, not a remote identity provider, admission service, or invented AI score. */
function dashboardMiniAward(s){
 const u=UNIVERSITY[s.uni],m=statusFor(s),tracked=state.apps.some(a=>a.id===s.id),chosen=state.compare.includes(s.id);
 return '<article class="dash-award"><div class="dash-award-top"><div class="dash-uni-mark">'+esc(u.mark)+'</div>'+
 '<span class="dash-status '+esc(m.key)+'">'+esc(m.label)+'</span></div>'+
 '<div class="dash-award-meta">'+esc(s.uni)+' · '+esc(u.place)+'</div>'+
 '<h3>'+esc(s.name)+'</h3><p class="dash-funding">'+esc(s.benefit)+'</p>'+
 '<p class="dash-award-reason">'+esc(m.reason)+'</p>'+
 '<div class="dash-award-actions"><button class="dash-small-button '+(tracked?'is-added':'')+'" data-track="'+esc(s.id)+'">'+(tracked?'✓ Open tracker':'+ Track')+'</button>'+
 '<button class="dash-small-button '+(chosen?'is-added':'')+'" data-compare="'+esc(s.id)+'">'+(chosen?'✓ Selected':'⊞ Compare')+'</button></div>'+
 '<div class="dash-source">'+sourceLink(s,'Official award rules ↗')+'</div></article>';
}
function dashboardApplication(a){
 const s=byId(a.id);if(!s)return '';
 const done=TASKS.filter(t=>Array.isArray(a.tasks)&&a.tasks.includes(t)).length;
 return '<button class="dash-list-row" data-view="applications">'+
 '<span class="dash-row-icon">'+esc(UNIVERSITY[s.uni].mark)+'</span>'+
 '<span class="dash-row-info"><strong>'+esc(s.name)+'</strong><small>'+esc(s.uni)+' · '+esc(a.stage||'Researching')+'</small>'+
 '<span class="dash-tiny-track"><i style="width:'+(done/TASKS.length*100)+'%"></i></span></span>'+
 '<span class="dash-row-end">'+done+'/'+TASKS.length+' ↗</span></button>';
}
function dashboard(){
 const p=state.profile,appCount=state.apps.length,selected=state.compare.length,filled=completeness();
 const candidates=SCHOLARSHIPS.filter(s=>['kaist-ug','unist-ug','ku-anam'].includes(s.id));
 const checkCount=SCHOLARSHIPS.filter(s=>statusFor(s).key==='good').length;
 const firstName=p.name?esc(p.name.trim().split(/\s+/)[0]):'';
 const actionRows=state.apps.filter(a=>a.date&& /^\d{4}-\d{2}-\d{2}$/.test(a.date)).slice().sort((a,b)=>a.date.localeCompare(b.date)).slice(0,4);
 const matched=state.compare.map(byId).filter(Boolean);
 const profileButton='<button class="btn btn-primary" data-view="eligibility">'+(filled>=5?'Review my profile':'Set up my profile')+' <span aria-hidden="true">↗</span></button>';
 const greeting='<div class="dash-greeting"><span class="dash-greeting-dot"></span>'+(firstName?'Welcome back, '+firstName+'.':'YOUR PERSONAL SCHOLARSHIP WORKSPACE')+'</div>';
 const hero='<section class="dash-hero"><div class="dash-hero-copy">'+greeting+
  '<div class="hero-overline"><span class="hero-overline-line"></span>UNIVERSITY SCHOLARSHIPS <span class="hero-overline-tail">/ A WORKSPACE FOR KOREA</span></div>'+
  '<h1>Your next chapter,<br><em>starts here.</em></h1>'+
  '<p>Discover source-linked Korean university awards, understand published criteria, and keep every application moving — in one focused workspace.</p>'+
  '<div class="dash-hero-actions">'+profileButton+'<button class="dash-hero-secondary" data-view="discover">Explore awards <span aria-hidden="true">↗</span></button></div>'+
  '</div><aside class="dash-plan" aria-label="Your scholarship plan"><div class="dash-plan-overline"><span>YOUR SCHOLARSHIP PLAN</span><span aria-hidden="true">✳</span></div>'+
  '<h2>Small steps.<br><em>Big possibilities.</em></h2><p>Everything you need to organize your university funding search.</p>'+
  '<div class="dash-plan-progress"><span>PROFILE ESSENTIALS</span><strong>'+filled+' / 5</strong></div>'+
  '<div class="dash-progress" role="progressbar" aria-label="Profile completion" aria-valuemin="0" aria-valuemax="5" aria-valuenow="'+filled+'"><span style="width:'+(filled*20)+'%"></span></div>'+
  '<div class="dash-plan-steps"><button data-view="eligibility"><span class="dash-step-index">01</span><span><strong>'+(filled>=5?'Review your profile':'Make it yours')+'</strong><small>'+filled+'/5 profile essentials</small></span><b>↗</b></button>'+
  '<button data-view="applications"><span class="dash-step-index">02</span><span><strong>Keep applications moving</strong><small>'+appCount+' application'+(appCount===1?'':'s')+' tracked</small></span><b>↗</b></button>'+
  '<button data-view="compare"><span class="dash-step-index">03</span><span><strong>See the complete picture</strong><small>'+selected+'/3 awards shortlisted</small></span><b>↗</b></button></div>'+
  '<div class="dash-plan-bottom"><span><span class="pulse"></span>LOCAL, PRIVATE PREVIEW</span><span>NON-GKS · KOREA</span></div></aside></section>';
 const statCard=(icon,n,label,detail,target)=>'<button class="dash-stat" data-view="'+target+'"><span class="dash-stat-icon">'+icon+'</span><span class="dash-stat-number">'+n+'</span><span class="dash-stat-text">'+label+'</span><span class="dash-stat-detail">'+detail+'</span><span class="dash-stat-arrow">↗</span></button>';
 const stats='<div class="dash-stats">'+statCard('◈',SCHOLARSHIPS.length,'Curated awards','4 university sources','discover')+
 statCard('▤',appCount,'My applications','Tracked individually','applications')+
 statCard('⊞',selected,'In comparison','Up to 3 awards','compare')+
 statCard('◎',checkCount,'Criteria met','Not an admission verdict','eligibility')+'</div>';
 const intro='<div class="dash-snapshot-note"><span class="dash-snapshot-dot"></span><strong>Independent preview</strong><span>No real sign-in or university submission. University scholarship snapshot checked '+SOURCE_DATE+'; verify your intake on official pages.</span>'+
 (state.demo?'<button id="clear-demo" type="button">Clear example data ↗</button>':(!filled&&!appCount&&!selected?'<button id="load-demo" type="button">Explore a sample workspace ↗</button>':''))+'</div>';
 const scholarships='<section class="dash-panel dash-wide"><div class="dash-panel-header"><div><span class="dash-section-kicker">EXPLORE THE POSSIBILITIES</span><h2>University scholarship spotlight</h2><p>Officially sourced awards, with transparent criteria insights.</p></div><button class="dash-text-button" data-view="discover">View all '+SCHOLARSHIPS.length+' <span>↗</span></button></div>'+
 '<div class="dash-award-grid">'+candidates.map(dashboardMiniAward).join('')+'</div><p class="dash-panel-foot">These are example entries from the curated catalogue, not a ranking or acceptance prediction. GKS entries are excluded.</p></section>';
 const progress=appCount?state.apps.slice(0,4).map(dashboardApplication).join(''):'<div class="dash-empty"><span class="dash-empty-symbol">▤</span><strong>Applications will appear here.</strong><p>Track an award to manage its checklist, documents and next actions.</p><button class="btn btn-small btn-primary" data-view="discover">Browse scholarships ↗</button></div>';
 const applications='<section class="dash-panel"><div class="dash-panel-header"><div><span class="dash-section-kicker">YOUR PROGRESS</span><h2>Application desk</h2></div><button class="dash-text-button" data-view="applications">Open tracker ↗</button></div><div class="dash-list">'+progress+'</div></section>';
 const shortlist=matched.length?matched.map(s=>'<div class="dash-list-row dash-nonbutton"><span class="dash-row-icon">'+esc(UNIVERSITY[s.uni].mark)+'</span><span class="dash-row-info"><strong>'+esc(s.name)+'</strong><small>'+esc(s.uni)+' · '+esc(s.benefit)+'</small></span>'+sourceLink(s,'↗')+'</div>').join(''):'<div class="dash-empty"><span class="dash-empty-symbol">⊞</span><strong>Bring your shortlist together.</strong><p>Compare up to three awards by tuition, funding, requirements and continuation rules.</p><button class="btn btn-small" data-view="discover">Select awards ↗</button></div>';
 const shortlistPanel='<section class="dash-panel"><div class="dash-panel-header"><div><span class="dash-section-kicker">SIDE BY SIDE</span><h2>Your comparison</h2></div><button class="dash-text-button" data-view="compare">Compare ↗</button></div><div class="dash-list">'+shortlist+'</div></section>';
 const actionData=actionRows.length?actionRows.map(a=>{let s=byId(a.id);return '<button class="dash-list-row" data-view="applications"><span class="dash-date">'+esc(a.date.slice(5,7))+'<strong>'+esc(a.date.slice(8))+'</strong></span><span class="dash-row-info"><strong>'+esc(s.name)+'</strong><small>Your next action date · '+esc(a.date)+'</small></span><span class="dash-row-end">↗</span></button>';}).join(''):'<div class="dash-empty dash-compact-empty"><span class="dash-empty-symbol">◷</span><strong>No action dates added yet.</strong><p>Add dates in your application tracker. Official admissions deadlines are not guessed.</p><button class="btn btn-small" data-view="applications">Manage dates ↗</button></div>';
 const actions='<section class="dash-panel"><div class="dash-panel-header"><div><span class="dash-section-kicker">WHAT’S NEXT</span><h2>Next action dates</h2></div><button class="dash-text-button" data-view="applications">All applications ↗</button></div><div class="dash-list">'+actionData+'</div></section>';
 const detail='<section class="dash-panel"><div class="dash-panel-header"><div><span class="dash-section-kicker">MY DETAILS</span><h2>Your study direction</h2></div><button class="dash-text-button" data-view="eligibility">Edit profile ↗</button></div>'+
 '<div class="dash-profile-info"><div><span>Study level</span><strong>'+esc(p.degree||'Not set')+'</strong></div><div><span>Intended major</span><strong>'+esc(p.major||'Not set')+'</strong></div><div><span>Language route</span><strong>'+esc(p.language||'Not decided')+'</strong></div><div><span>Target intake</span><strong>'+esc(p.intake||'Not decided')+'</strong></div></div><div class="dash-profile-bottom">'+filled+'/5 essentials completed <span class="dash-inline-bar"><i style="width:'+(filled*20)+'%"></i></span></div></section>';
 $('#view-root').innerHTML=shell(hero+stats+intro+'<div class="dash-dashboard-grid">'+scholarships+applications+shortlistPanel+actions+detail+'</div>');
}
...
function render(){syncShell();({dashboard,discover,eligibility,applications:apps,compare})[view]();}
document.addEventListener('click',e=>{
 if(e.target.closest('#load-demo')){if(state.apps.length||completeness()){if(!confirm('Replace this preview workspace with clearly labelled example data?'))return;}state={profile:{name:'Demo applicant',nationality:'Example country',degree:'Undergraduate',major:'AI / Computer Science',gpa:'',scale:'',topik:'4',ielts:'',language:'Korean track',intake:'Fall 2027',stemEvidence:false},apps:[{id:'kaist-ug',stage:'Preparing documents',intake:'Fall 2027',date:'',tasks:['Check admission guide'],notes:''},{id:'unist-ug',stage:'Researching',intake:'Fall 2027',date:'',tasks:[],notes:''}],compare:['kaist-ug','unist-ug'],demo:true};store();render();toast('Example workspace loaded. No actual applications were submitted.');return;}
 if(e.target.closest('#clear-demo')){if(!confirm('Clear only this dashboard preview’s example data and edits?'))return;state={profile:{...DEFAULT_PROFILE},apps:[],compare:[],demo:false};store();render();toast('Example workspace cleared.');return;}
...
window.addEventListener('hashchange',()=>{const v=location.hash.slice(1);if(['dashboard','discover','eligibility','applications','compare'].includes(v)&&v!==view){view=v;render();}});
render();
