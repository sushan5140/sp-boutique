...
function header(title,subtitle,action){return '<div class="hero-row"><div>'+sectionEyebrow('KOREA · NON-GKS UNIVERSITY AWARDS')+'<h1>'+title+'</h1><p>'+subtitle+'</p></div><div class="hero-actions">'+(action||'')+'</div></div>';}
...
function discover(){
 ...
 let html=header('Find funding that fits <br>your next chapter.','A focused workspace for university-funded opportunities in South Korea. Explore official awards, check published criteria, and organize applications — separate from GKS.',profileButton)+stats()+
 ...
 '<div class="section-heading"><div><div class="eyebrow">01 / OPPORTUNITY FINDER</div><h2>Explore scholarships</h2><p>Source-linked university awards. Your criteria insights appear on every card.</p></div><div class="no-gks">GKS programmes excluded ✳</div></div>'+
...
function eligibility(){
 ...
 let html=header('A profile that works <br>across universities.','Tell us a little about your study plans. We compare only explicitly published award criteria — never invent admission probabilities.','<button class="btn" data-view="discover">← Browse scholarships</button>')+
...
function apps(){
 let intro=header('Every application, <br>one clear workspace.','Move each opportunity through research, preparation and submission. Your dates, notes and checklist stay on this device.','<button class="btn btn-primary" data-view="discover">＋ Discover an award</button>');
...
function compare(){
 const title=header('Put the details <br>side by side.','Select up to three university awards. See documented funding, selection route, continuation and your personal criteria signal without misleading ranking or guessed eligibility.','<button class="btn" data-view="discover">← Find awards</button>');
